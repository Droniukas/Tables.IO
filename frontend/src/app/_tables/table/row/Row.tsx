...
function Row(props: RowProps) {
  ...
  const mapDatacell = (datacell: TableDatacellDto, customOnSave?: Function) => {
  ...
  return (
    <tr className={`${styles.dataRow} ${styles[rowColorClass]}`}>
      {row.isRowForAddingNewRow ? (
        ...
        <>{(row as TableRowDto).datacells.map((datacell) => mapDatacell(datacell, updateDatacellValueById))}</>
      )}
    </tr>
  );
}
